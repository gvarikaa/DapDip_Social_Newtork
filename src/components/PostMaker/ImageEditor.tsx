...
const ImageEditor = ({
  ...
  settings,
  setSettings,
  fileIndex = 0, // დავამატეთ fileIndex პარამეტრი
}: {
  onClose: () => void;
  previewURL: string;
  settings: {
  ...
  >;
  fileIndex?: number; // არჩევითი ინდექსი რედაქტირებისთვის
}) => {
  ...
  return (
    <div className="fixed w-screen h-screen left-0 top-0 bg-black bg-opacity-75 z-10 flex items-center justify-center">
      <div className="bg-black rounded-xl p-12 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <h1 className="font-bold text-xl">
              {fileIndex > 0 ? `ფაილი #${fileIndex + 1} - რედაქტირება` : "მედიის პარამეტრები"}
            </h1>
          </div>
        ...
        {/* გადავიყვანოთ ყველა ფაილზე */}
        <div className="flex items-center justify-center mt-4">
          <label className="flex items-center text-textGray">
            <input
              type="checkbox"
              className="mr-2"
              onChange={(e) => {
                // თუ ჩეკბოქსი ჩართულია, გამოიყენე ეს პარამეტრები ყველა ფაილზე
                // ეს ლოგიკა Share კომპონენტში უნდა დაემატოს
              }}
            />
            ეს პარამეტრები გამოიყენე ყველა ფაილზე
          </label>
        </div>
      </div>
    </div>
